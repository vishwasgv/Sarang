...
- [ ] Full `npm run test:e2e` re-run — final confirmation run in progress as of this checklist update (the two suites above were already individually re-verified clean twice each); this line gets checked once the full-suite run's actual final count is in hand.
