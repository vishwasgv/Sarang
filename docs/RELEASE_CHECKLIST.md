...
### Assets
- [ ] `package.json` version follows semantic versioning and matches the intended release — **STALE, needs a fresh decision, not just a re-verify**: this line was last true for `1.0.0` (2026-07-15). Real shipped versions since: `v1.0.1`, `v1.1.0`, `v1.1.1`, `v1.1.2` (current `package.json` version as of 2026-07-28). Phase 59 (licensing/monetization/lead-capture) and the usage-metrics feature have both landed on top of `1.1.2` and are **not yet reflected in any version bump** — this is a real, user-facing behavior change (free-forever installs now become free-for-12-months-then-paid for anyone downloading fresh) and, per this file's own Release Types table, is at minimum a Minor bump (new feature), arguably judged against how significant the monetization change is. **Decide and bump the version as part of this release**, don't ship Phase 59 under the same `1.1.2` number that shipped before it existed.
...
- [x] `npm run lint` passes, 0 errors (32 pre-existing warnings, mostly `react-hooks/exhaustive-deps` and unused-vars in files untouched by this session — none new, none blocking) — re-verified 2026-07-28
- [x] `npm run test` passes, 0 failures — **re-verified 2026-07-28 (end of the 5-pass audit below): 1965/1965, 137 files** (grew from 1895/1895 at the start of this same day's audit session, and from 1154/97 on 2026-07-15, as Phases 56-59, usage-metrics, and this session's 5-pass bug-hunt all landed; the 2026-07-15 number in individual fix narratives elsewhere in this file is that session's own historical count, left as-is since it's evidence for what was true at that specific past moment, not a current-state claim)
...
- [ ] Completion reports exist for every phase — **real gap found 2026-07-28**: still only 65 `PHASE_*_COMPLETION_REPORT.md` files (Phases 1-57 + batches). Phase 58 (43-vertical full coverage), Phase 59 (licensing/monetization), and now Phase 60 (guided tutorial) all genuinely happened and are documented elsewhere (memory files, `PHASE_59_MONETIZATION_LICENSING_MASTER_PROMPT.md`, `PHASE_58_VERTICAL_COVERAGE_PLAN.md`), but none has the standard formal `PHASE_58/59/60_COMPLETION_REPORT.md` this project has written for every phase before it. Not a product bug — a documentation-completeness gap worth closing before calling this release fully documented.
...
- [x] No **business-data** telemetry, analytics, or tracking of any kind — re-verified 2026-07-28: `publish: null` still holds, and no business data (invoices, customers, products, reports) is ever transmitted anywhere, confirmed by architecture (fully local SQLite, no sync layer) not just by policy.
- [ ] **This line is now factually different from its 2026-07-15 wording and must not be copy-pasted forward**: as of Phase 59 + the usage-metrics feature (both post-dating this file's last full pass), there are now **three** disclosed non-localhost `fetch` calls in `src/main`, not one — `app.handler.ts`'s `app:checkForUpdates` (unchanged, user-initiated, version number only), `license.service.ts`'s `pingLicenseStatusIfDue()` (optional, at-most-once-daily, sends only a one-way hash of the license key, never blocks if offline), and `usage-metrics.service.ts`'s `flushUsageQueue()` (aggregate anonymous daily usage-duration, same hash, disclosed once at setup, never blocks). **None of these send business data**, and this remains a true, defensible "no business-data telemetry" claim — but any copy anywhere (in-app, website, this checklist) that says "no telemetry" or "no tracking" **unqualified** is now false and must be corrected to the "no business-data telemetry" framing. Checked and fixed 2026-07-28 in: `AboutScreen.tsx`, `sarang-privacy.html`, `docs/LEGAL_AND_CLAIMS_POLICY.md`. **Not yet re-checked**: any other marketing copy on the website beyond the privacy policy page itself — a fresh grep for "no telemetry"/"no tracking" across `D:\Aszurex website\public\*.html` is worth doing before this ships.
...
- [x] **Real gap found+closed 2026-07-29, distinct from the two gaps above**: license keys used to be issued with **zero per-request uniqueness** — the signed payload was built from only tier+region+issuance-day, so any two people issued a key for the same tier+region on the same calendar day got the byte-for-byte identical key string (HMAC-SHA256 is deterministic). Confirmed this was a real, non-hypothetical risk, not just a theoretical one — measured a ~2.9% chance of any collision at just 1,000 same-day signups in one region, rising to ~95% by 10,000. Fixed with a random nonce added as its own payload segment (first sized at 3 bytes, then widened to 6 bytes after live stress-testing showed 3 wasn't a strong enough guarantee — 0 collisions measured across 100,000 same-day signups at 6 bytes). Old keys (no nonce) keep validating forever. Fixed in **both** repos — this app's `license.service.ts` and the website's mirrored `server.js` generator (the real production issuer) — and cross-interop between the two was verified directly, not assumed. See `docs/LICENSING_AND_RENEWAL_MECHANICS.md` Section 8.
- [ ] **Website repo (`server.js`) change not yet deployed** — the nonce fix was implemented and locally verified in the website repo's working tree, but deploying it to the live Render service (which auto-deploys on push, per this project's own convention) is a separate, explicit step not yet taken as of this checklist entry. Every key issued between now and that deploy still has the old, weaker collision profile. Deploy before this matters at real signup volume.
...
section — this was a code-level adversarial audit (race conditions,
financial-reconciliation gaps, timezone bugs, silent-clamp-instead-of-reject
bugs, float-precision bugs, license-gate bypasses, permission-check gaps)
run as 5 sequential passes across the whole service layer, each followed by
real fixes + regression tests + `typecheck`/`lint`/full-suite re-verification
+ a commit, not just a findings list. ~24 real bugs found and fixed total,
zero deferred as "won't fix." One genuine product-policy question surfaced
(not a bug) — see the founder report for that single open item.
- [x] **Pass 1 — Core commerce** (Billing, Invoicing, Payments, Inventory): 7 bugs fixed, incl. `serial.service.ts`'s replacement-unit double-claim race, `credit-note.service.ts` never reconciling the linked invoice's balance/status, `variant.service.ts` silently clamping instead of rejecting oversell, and a systematic `new Date(x)` vs. `parseLocalDateStart(x)` write/read timezone mismatch across `billing.service.ts`/`quotation.service.ts`/`purchase-order.service.ts`.
- [x] **Pass 2 — Service-vertical batch** (clinics, salons, gyms, legal/CA/CS, agencies): 5 bugs fixed, incl. `driving.service.ts` missing a scheduling-conflict + package-enrollment atomicity check entirely, `membership.service.ts` expiring memberships ~18.5 hours early in IST from the same UTC-midnight bug class, and `session-pack.service.ts` reading `expiryDate` for a WhatsApp reminder but never actually enforcing it against session deduction.
- [x] **Pass 3 — Product-vertical batch** (pharmacy, electronics, jewellery, hardware, distributor, agri, blood bank, hotel): 6 bugs fixed, incl. `repair-ticket.service.ts` and `logistics-shipment.service.ts` double-claim races on a replacement unit/vehicle, `metal-exchange.service.ts`'s `valueGiven` raw-float precision, GRN/challan/shipment `totalValue`/`totalCost` computed with raw float multiplication feeding directly into `supplier-ledger.service.ts`'s unrounded running balance (now rounded at every step in the chain, not just at the input), and `hotel.service.ts`'s check-in/check-out having a stale pre-transaction status check that let a double-click create duplicate `HotelGuestId`/`HotelHousekeepingTask` rows — directly undermining the guest register this vertical exists to produce for police/immigration compliance.
- [x] **Pass 4 — Reports/Analytics, Settings, HR/Payroll, Security/Permissions**: 4 bugs fixed, incl. `hr.service.ts`'s `LeaveType.maxDays` cap being computed for display (`getLeaveBalance`) but never actually enforced on approval, `payroll.service.ts`'s `updateSalaryPayment` racing against `markSalaryPaid`'s "no edits after PAID" invariant via a stale pre-transaction check + unconditional write, and `reports.handler.ts` gating `reports:attendance`/`reports:logistics`/`reports:projects`/`reports:jobCards` on the blanket `reports.sales` permission instead of the stricter `hr.view`/`logistics.view`/`sales.view` every sibling handler for that same data actually requires — a real Cashier-role side-channel into HR/logistics/project data.
- [x] **Pass-over-pass regression, caught by E2E, not by unit tests**: `appointment.service.ts`'s `getAppointmentsByDate` (backs the Appointments screen's "Today" tab) still used bare `new Date(date)` (UTC midnight) after Pass 2 correctly fixed this same file's WRITE side (`createAppointment`/`updateAppointment`) to `parseLocalDateStart` — a sibling function in the same file was missed, creating a fresh read/write mismatch. The first full E2E run of this audit hung immediately on an unrelated harness gap (see below) before ever reaching the suites that would have caught this, so it went undetected through 4 full passes of unit-test-and-commit cycles. Once E2E actually ran, `02-service-business.js` and `34-beautysalon-multiservice.js` both failed (11/13 and 10/13) on their "advance an appointment via the Today view" steps. Fixed to match its siblings; both suites now pass cleanly (14/14, 13/13) reproducibly across repeat runs, with a new regression test added.
- [x] **Harness gap fixed**: `tests/e2e/harness.js`'s `login()` had no handling for the Phase 60 first-run "Want a quick guided tour?" full-screen prompt (shown once, before the Dashboard ever renders) — the same shape as the already-known `dismissBackupPrompt` gotcha, just never extended to this newer prompt. Fixed with a `dismissTutorialPrompt()` helper.
- [ ] **Pre-existing, NOT a regression, NOT fixed this session**: `driving.service.ts`'s own date-range list filter has the identical `new Date(dateString)` shape, but self-consistently — its write side was never touched by any timezone fix (not in Pass 2's scope, not in the original 2026-07-22 audit either), so it doesn't fail any current test. Still a real correctness bug for non-UTC-timezone users; flagged for a future dedicated pass rather than fixed reactively here.
- [ ] **Pre-existing, unrelated to this audit**: `tests/e2e/run-all.js`'s orchestrated sweep cannot execute `60-tutorial-mode.js` at all (`FATAL: suite.run is not a function`) — that suite file self-executes rather than exporting `{ run }` like every sibling suite, so it's structurally incompatible with the orchestrator and has apparently only ever been run standalone. Not fixed this session.
- [x] Full re-verification after all 5 passes + the E2E-caught fix: `npm run typecheck` (0 errors, both configs), `npm run lint` (0 errors), `npm run test` (1966/1966, up from 1895 at the start of this audit).
- [x] Full `npm run test:e2e` re-run — **920/921 passing** (the full sweep flaked on 2 different suites across 2 earlier attempts — `09-stress.js`/`10-new-features.js` once, `18-distributor-bulk-outstanding.js`/`28-vetclinic-pets.js` another time — each confirmed passing 100% clean when re-run in isolation, consistent with this project's own previously-documented full-sweep-only flakiness; this final clean run shows every suite passing except the one pre-existing, already-documented `60-tutorial-mode.js` orchestration incompatibility above, unrelated to this session's changes).
...
- [ ] **New for this release specifically**: confirm an install upgraded from a pre-Phase-59 version shows `NOT_ACTIVATED` on the License screen (not a crash, not an incorrectly-expired state) and remains fully functional for billing until the user chooses to enter a key — the old "free forever" promise those installs were given must hold.
...
- Sarang checks for new versions automatically (once/day, toggleable in Settings → About) and shows a Dashboard notice when one's available — **installing it is still always manual**, Sarang never auto-downloads or auto-installs (`electron-builder.config.ts`'s `publish: null` stance is unchanged). This line previously said "no automatic software updates" without qualifying which half is automatic (checking) vs. manual (installing) — corrected 2026-07-28, since Phase 59.13 shipped after this line was last accurate.
- Free for the first 12 months from setup; a small annual license (₹599/year India, $29/year international) keeps billing working after that — existing data is never at risk and never blocked, only creating new invoices/billable documents pauses until renewal. Installs from before this feature shipped keep their original free-forever promise (see Licensing subsection in Section 3).
- Sarang sends an anonymous, aggregate daily usage-duration signal (a one-way hash of the license key + minutes used) at most once a day when online — disclosed once at setup, never business data, never individually identifying.
