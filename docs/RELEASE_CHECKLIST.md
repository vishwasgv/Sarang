...
### Assets
- [x] `package.json` version follows semantic versioning and matches the intended release — **Decided and bumped 2026-08-06: `1.1.2` → `1.2.0` (Minor).** This release carries Phase 59 (licensing/monetization/lead-capture) and the usage-metrics feature landing on top of `1.1.2` for the first time in a shipped installer, plus: the full 12-language translation pass closing out (all 12 non-English locales now complete, 0 missing/disguised-untranslated/UI-label-mismatch keys), a new WiFi-join QR feature for restaurant table ordering, and a large manual-content accuracy sweep (1,947 stale English UI-label fixes across all 12 languages' original chapters). Justifies Minor per this file's own Release Types table (new user-facing features + a real behavior change: free-forever installs now become free-for-12-months-then-paid for anyone downloading fresh from this release forward — old installs, v1.1.2 and earlier, remain permanently unaffected).
...
- [x] **Website repo (`server.js`) fix deployed 2026-07-29** — committed (`042d53f` on the aszurex website repo) and pushed to `main`, triggering Render's auto-deploy. Every license key issued from that point forward uses the 6-byte-nonce format.
