...
Written 2026-07-28, verified directly against the current code (not a summary of a design doc) as part of the pre-installer full audit. See `PHASE_59_MONETIZATION_LICENSING_MASTER_PROMPT.md` (repo root, one level up) for the original design spec this implements; this document describes **current, live behavior**, including one real gap found and closed during this audit.
