...
export function DebitNotesScreen() {
  ...
  // Itemization is create-only — debitNotes.update never accepted `items`
  // (see debit-note.validation.ts's own comment), so editing an existing
  ...
  async function handleSave() {
    ...
    if (!form.reason.trim()) { toastError(t('debitNotes.reasonAmountRequired')); return }
    if (useItems && validItems.length === 0) { toastError(t('debitNotes.reasonAmountRequired')); return }
    if (!useItems && !form.amount) { toastError(t('debitNotes.reasonAmountRequired')); return }
    setSaving(true)
    try {
          ...
          {!editTarget && (
            <div className="border-t border-slate-100 dark:border-slate-800 pt-4">
              <label className="flex items-center gap-2 cursor-pointer mb-3">
                ...
                <span className="text-sm font-medium text-dark dark:text-slate-100">{t('debitNotes.itemizeThisNote')}</span>
              </label>
              {useItems && (
