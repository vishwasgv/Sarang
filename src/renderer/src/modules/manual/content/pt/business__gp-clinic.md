...
Todo tipo de negócio baseado em serviço no Sarang — incluindo Clínica de Clínico Geral — parte dos mesmos quatro blocos de construção: **Appointments** (agendar e programar visitas), um **Service Catalog** (a lista de consultas e seus preços), **Provider Schedules** (qual médico está disponível quando), e uma **Notification Queue** automática que cuida dos lembretes sem que você precise enviá-los manualmente. O restante deste capítulo cobre o que é específico de uma clínica de clínico geral: notas de consulta e uma fila de senhas para pacientes sem hora marcada.
...
A tela **Token Queue** gerencia pacientes sem hora marcada do mesmo dia sem precisar de um agendamento pré-reservado. Ela mostra:
