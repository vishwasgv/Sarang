...
O tempo pode ser registrado tanto de dentro de um processo (na tela Legal Cases) quanto na tela independente **Time Tracking**, que lista cada entrada em todos os processos com data, membro da equipe, descrição, horas, taxa, e valor calculado. Filtre por equipe, período, ou status de faturamento. Selecione uma ou mais entradas não faturadas e clique em **Generate Invoice** para transformar diretamente as horas registradas em uma nota fiscal real para o cliente — entradas faturadas não podem mais ser editadas ou excluídas, mantendo intacto o rastro de faturamento.
