...
| Cost Price | What you pay per unit, **before tax** |
| Selling Price | What you charge per unit, **before tax** |
...
**Prices are entered before tax.** Sarang adds tax on top when you sell or buy. Example: selling price 100 with 18 percent tax sells at 118. If your shelf price already includes tax, work out the before-tax price first (118 divided by 1.18 gives 100). See *Guide: Tax and GST*.
...
Sarang will not let you sell more than you have. If a sale is blocked with *Insufficient stock*, either receive the purchase first or correct the stock count with a reason.
...
- **Counting stock**: after a physical count, use **Adjust stock** on each product and enter the counted quantity with the reason "Physical stock count". (A dedicated stock-count session with a variance report is planned.)
...
**Inventory** shows the value of each product (quantity x average cost). Reports show valuation by method (average, FIFO and others where enabled) and by location. Costs from freight or duty entered as **landed cost** on a purchase raise the cost of those items.
...
| Entering selling price including tax | Customers are charged tax twice | Enter the before-tax price |
