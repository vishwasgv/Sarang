...
Yes, **you can switch business types at any time** — pick a different template and click "Apply Template" (a confirmation dialog will ask you to confirm the switch first). This changes your sidebar navigation and feature set immediately — no restart required — and **all existing data is preserved**: every customer, product, invoice, and record stays exactly as it is, only which features are visible changes. Since only one business type is active at a time, its own feature set is what you see — a Retail shop that switches to Distributor won't see Retail's Returns Workflow module while it's on Distributor. This is **not data loss**: each business type remembers its own feature toggles independently, so switching back to Retail later restores Returns Workflow (and every other toggle) exactly as you'd left it.

One real consequence worth knowing: most professional/service business types (Lawyer, Architect, CA Firm, and similar) are English-only. If you're using Sarang in another language, switching into one of those will switch your interface to English — switching back to a product-based business type (Retail, Hardware, Pharmacy, etc.) restores full multi-language support.
