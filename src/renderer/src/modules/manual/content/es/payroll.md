...
El aviso que se muestra debajo de la lista de deducciones vale la pena leerlo: Sarang nunca aplica automáticamente las reglas oficiales del gobierno para PF/ESI/Impuesto Profesional — esas cifras deben provenir de su propio contador o de las reglas de nómina aplicables, y se ingresan aquí como simples líneas de deducción.

Si ha ingresado su propio % de PF, % de ESI (con un tope salarial opcional) y el importe del Impuesto Profesional en **Configuración → Perfil del Negocio**, aparece un enlace **Sugerir según tasas legales** junto al encabezado de Deducciones. Al tocarlo se rellenan previamente líneas de deducción calculadas según sus propias tarifas configuradas sobre el Salario Básico de ese empleado — reemplaza cualquier línea existente con el mismo nombre, así que volver a ejecutarlo después de cambiar una tarifa no deja un duplicado — pero nunca se guarda por sí solo; usted debe revisar cada línea sugerida y tocar **Guardar** usted mismo. Si deja una tarifa en blanco en Configuración, ese rubro estatutario simplemente nunca se sugiere.
