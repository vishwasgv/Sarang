...
El informe de **Vencimiento de Lotes** (Informes → Batch Expiry) convierte esos mismos datos en dinero: además de cuántos lotes hay en cada ventana de vencimiento, ahora muestra el **valor en riesgo** real en cada categoría, para que pueda ver de un vistazo no solo "12 lotes vencen pronto", sino exactamente cuánto valor de stock representa eso — y una cifra separada de "Valor en Riesgo" totaliza lo que aún es recuperable (el stock ya vencido es una pérdida irreversible, no algo que un reabastecimiento o una devolución puedan solucionar, por eso se excluye de ese total).
