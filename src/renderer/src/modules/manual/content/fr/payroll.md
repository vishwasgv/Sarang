...
L'avertissement affiché sous la liste des retenues mérite d'être lu : Sarang n'applique jamais automatiquement les règles officielles du gouvernement pour le PF/ESI/la Taxe Professionnelle — ces chiffres doivent provenir de votre propre comptable ou des règles de paie applicables, saisis ici comme de simples lignes de retenue.

Si vous avez renseigné votre propre % de PF, % d'ESI (avec un plafond salarial optionnel) et le montant de la Taxe Professionnelle dans **Paramètres → Profil de l'entreprise**, un lien **Suggérer selon les taux statutaires** apparaît à côté de l'en-tête Retenues. Appuyer dessus pré-remplit des lignes de retenue calculées à partir de vos propres taux configurés, appliqués au Salaire de Base de cet employé — cela remplace toute ligne existante portant le même nom, donc relancer l'opération après avoir modifié un taux ne laisse pas de doublon derrière — mais cela n'enregistre jamais tout seul ; vous devez toujours vérifier chaque ligne suggérée et appuyer vous-même sur **Enregistrer**. Laissez un taux vide dans Paramètres et cette rubrique statutaire n'est tout simplement jamais suggérée.
