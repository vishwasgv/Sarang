import { listDrawingRevisions, createDrawingRevision, updateDrawingRevision, deleteDrawingRevision, issueNewRevision, getRevisionHistory, getOrphanedSupersededDrawings } from '../../services/drawing-revision.service'
...
export function register(handle: HandleFn): void {
  ...
  handle('drawingRevision:orphanedSuperseded', async (payload) => {
    const deny = await requirePermission('billing.view'); if (deny) return deny
    const { projectId } = payload as { projectId: string }
    if (!projectId) return { success: false, error: { code: 'VAL-001', message: 'Project ID is required.' } }
    return getOrphanedSupersededDrawings(projectId)
  })
}
