...
export function register(handle: HandleFn): void {
  ...
  handle('quotations:convertToSalesOrder', async (id) => {
    const deny = await requirePermission('billing.create'); if (deny) return deny
    const bad = validateId(id, 'quotation ID'); if (bad) return bad
    const session = getCurrentSession()
    if (!session) return { success: false, error: { code: 'AUTH-001', message: 'Not authenticated.' } }
    return quotationService.convertToSalesOrder(id as string, session.userId)
  })
  ...
  handle('quotations:print', async (id) => {
    ...
    return printHtml(html, 'sarang_qt', { silent: false, printBackground: true, color: !isReceipt })
  })
  ...
  handle('quotations:printReceipt', async (payload) => {
    ...
    return printHtml(html, 'sarang_qt_rcpt', { silent: false, printBackground: true })
  })
  ...
  handle('quotations:exportPdf', async (id) => {
    const deny = await requirePermission('billing.printInvoice'); if (deny) return deny
    const bad = validateId(id, 'quotation ID'); if (bad) return bad
    const qtRes = await quotationService.getById(id as string)
    if (!qtRes.success) return qtRes
    const profile = await getPrisma().businessProfile.findFirst()
    const html = await printService.generateQuotationHtml(qtRes.data as Parameters<typeof printService.generateQuotationHtml>[0], profile as Parameters<typeof printService.generateQuotationHtml>[1])
    const quotationNumber = (qtRes.data as { quotationNumber: string }).quotationNumber
    const result = await exportToPdf({ html, filename: `Quotation-${quotationNumber}.pdf` })
