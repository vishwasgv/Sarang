    ...
    const product = await db.$transaction(async (tx) => {
      ...
      const p = await tx.product.create({
        data: {
          ...
          ...(payload.taxCategory ? { taxCategory: payload.taxCategory } : {}),
          imagePath: payload.imagePath || null,
          gender: payload.gender ?? null,
          foodType: payload.foodType ?? null,
          season: payload.season?.trim() || null,
          recommendedCrop: payload.recommendedCrop?.trim() || null,
          isPrescriptionRequired: payload.isPrescriptionRequired ?? false,
          isScheduleH1X: payload.isScheduleH1X ?? false,
          ...
          sellByLength: payload.sellByLength ?? false,
          lengthUnit: payload.sellByLength ? payload.lengthUnit : null,
          pricePerLengthUnit: payload.sellByLength ? payload.pricePerLengthUnit : null,
