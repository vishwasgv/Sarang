...
async function run() {
  ...
  try {
    ...
    await r.step('create-rental-unit-product', async () => {
      ...
      await modal.getByLabel(/Selling price/i).fill('2000')
      await modal.getByLabel(/Cost price/i).fill('1500')
    ...
    await r.step('create-rental-bulk-product', async () => {
      ...
      await modal.getByLabel(/Selling price/i).fill('500')
      await modal.getByLabel(/Cost price/i).fill('300')
      await modal.locator('text=This item can be rented out').click()
      await page.waitForTimeout(300)
